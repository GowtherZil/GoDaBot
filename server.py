...
# Cypher 1150289891:AAE568Kp01R-9QqU3PQ0UDWAePVIiSIaIoE
# pyTeleg 1213090474:AAE5oNKw9SS4jfBMEsGGdO1tr86N8jB80k0
bot = telebot.TeleBot("1213090474:AAE5oNKw9SS4jfBMEsGGdO1tr86N8jB80k0")
#                                                                                                                                                                                                                                                                                                                                                                                                                               Gaby      Zod        Jesu                                                                                                                                                                                   alt Gaby
listaPermitidos=[716072728 , 792560181 , 731287202 , 911727415 , 684813021 ,929349801 ,  700105381 , 1048144798 , 714046873 , 1065343191 , 957307955 , 957903181 , 408101137 , 1103706614 , 964707556 , 790294138 , 1163556487, 1163556487, 1157196238, 874408644 , 1125111091 , 806342188, 1177893995, 1253189549, 1076609601, 1002169688, 1241596994, 764720084, 1090814880, 1109719043, 1140054736, 1174056282, 1216519892,696138098,977103637,1105174386 , 1014257191, 1027745207, 968100657, 1081747993,716273630,1062486659,854575960,825606513,1138317032,1077344630,730612359,1194510670,970260240,1036991409,1250272620,935801093,991929844, 563136452,891431070,806667545]
#                                                                                                                          KMH              GNR
listaGruposPermitidos = [-1001154054290,-1001434482847, -1001347769726,-1001472465292, -1001447765240,-1001495225305,-1001224449552,-1001434482837]

lista_Permitidos_Info = [716072728 , 911727415, 1157196238, 684813021, 792560181]
...
botonera_de_howtos.add(forestHTButton,swampHTButton,valleyHTButton,saveHTButton,hideHTButton,ghideHTButton,updateHTButton,workHTButton,feedbackHTButton,stockHTButton,backButton)
...
def echo_all(message):
    ...
    if(message.text == "Comandos disponibles" and sent==0 ):
        ...
        /valley
        \n
        Estos ultimos 3 te mostraran los drops que encontrarias en cada una de esas quest si las hicieses en el preciso momento en el que los consultaste. Ademas te mostrare en el final del sms el tiempo que tienes para mover tu flacido culo fofo * simula risa * para dicha quest antes de que el drop cambie
        \n
    ...
    if("/say" in message.text and message.from_user.id == usuarito):
    ...
    if("/tell" in message.text and message.from_user.id == usuarito):
